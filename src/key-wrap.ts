 ...
 */
export async function generateMasterKeyPair(kem?: Kem): Promise<MasterKeyPair> {
...
export async function wrapKey(
  ...
  if (masterPublicKey.length !== kem.publicKeySize) {
    throw new InvalidInputError(
      `masterPublicKey must be ${String(kem.publicKeySize)} bytes for ${kem.id}, got ${String(masterPublicKey.length)}`,
    );
  }
...
export async function unwrapKey(
  ...
  if (wrapped.kemId !== kem.id) {
    throw new InvalidInputError(
      `KEM mismatch: wrapped key uses "${wrapped.kemId}" but unwrap was called with "${kem.id}"`,
    );
  }

  if (wrapped.enc.length !== kem.encSize) {
    throw new InvalidInputError(
      `enc must be ${String(kem.encSize)} bytes for ${kem.id}, got ${String(wrapped.enc.length)}`,
    );
  }
