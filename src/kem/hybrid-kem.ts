...
export const HYBRID_X25519_MLKEM768: Kem = {
  ...
  async generateKeyPair(): Promise<KemKeyPair> {
    ...
    const x25519kp = await globalThis.crypto.subtle.generateKey(
      { name: 'X25519' },
      true,
      ['deriveBits'],
    ) as CryptoKeyPair;
  ...
  async encapsulate(publicKey: Uint8Array): Promise<EncapsulationResult> {
    ...
    const pkXKey = await globalThis.crypto.subtle.importKey(
      'raw',
      pkX as Uint8Array<ArrayBuffer>,
      'X25519',
      true,
      [],
    );
  ...
  async decapsulate(enc: Uint8Array, privateKey: Uint8Array): Promise<Uint8Array> {
    ...
    // X25519 DH(skX, ctX)
    const skXKey = await globalThis.crypto.subtle.importKey(
      'pkcs8',
      wrapPkcs8(skX) as Uint8Array<ArrayBuffer>,
      'X25519',
      false,
      ['deriveBits'],
    );
    const ctXKey = await globalThis.crypto.subtle.importKey(
      'raw',
      ctX as Uint8Array<ArrayBuffer>,
      'X25519',
      true,
      [],
    );
