...
async function importPrivateKey(rawSk) {
    return globalThis.crypto.subtle.importKey('pkcs8', wrapPkcs8(rawSk), 'X25519', false, ['deriveBits']);
}
async function importPublicKey(rawPk) {
    return globalThis.crypto.subtle.importKey('raw', rawPk, 'X25519', true, []);
}
/** Test-only: raw X25519 DH. */
export async function _dh(rawSk, rawPk) {
    const sk = await importPrivateKey(rawSk);
    const pk = await importPublicKey(rawPk);
    return new Uint8Array(await globalThis.crypto.subtle.deriveBits({ name: 'X25519', public: pk }, sk, 256));
}
/**
 ...
 */
export async function _extractAndExpand(dh, kemContext) {
...
export const DHKEM_X25519 = {
    ...
    async generateKeyPair() {
        const keyPair = await globalThis.crypto.subtle.generateKey({ name: 'X25519' }, true, ['deriveBits']);
    ...
    async encapsulate(publicKey) {
        ...
        const dh = new Uint8Array(await globalThis.crypto.subtle.deriveBits({ name: 'X25519', public: pkR }, ephemeral.privateKey, 256));
        // kem_context = enc || pkR
        const enc = pkE;
    ...
    async decapsulate(enc, privateKey) {
        ...
        const dh = new Uint8Array(await globalThis.crypto.subtle.deriveBits({ name: 'X25519', public: pkE }, skR, 256));
        // kem_context = enc || pkR
        const kemContext = concat(enc, pkR);
        return _extractAndExpand(dh, kemContext);
    },
};
//# sourceMappingURL=dhkem-x25519.js.map