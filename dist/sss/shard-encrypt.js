...
import { sign, verify, ALGORITHM_ID, SECRET_KEY_BYTES, PUBLIC_KEY_BYTES, SIGNATURE_BYTES, } from '../sig/ml-dsa.js';
 ...
 */
export async function distributeShard(shard, custodianPublicKey, ownerSigningKey, options) {
    // Validate shard
    if (shard.value.length !== SHARD_VALUE_BYTES) {
        throw new InvalidInputError(`shard value must be ${String(SHARD_VALUE_BYTES)} bytes, got ${String(shard.value.length)}`);
    }
    if (shard.mac.length !== SHARD_MAC_BYTES) {
        throw new InvalidInputError(`shard mac must be ${String(SHARD_MAC_BYTES)} bytes, got ${String(shard.mac.length)}`);
    }
    // Validate signing key
    if (ownerSigningKey.length !== SECRET_KEY_BYTES) {
        throw new InvalidInputError(`ownerSigningKey must be ${String(SECRET_KEY_BYTES)} bytes, got ${String(ownerSigningKey.length)}`);
    }
    ...
    if (custodianPublicKey.length !== kem.publicKeySize) {
        throw new InvalidInputError(`custodianPublicKey must be ${String(kem.publicKeySize)} bytes for ${kem.id}, got ${String(custodianPublicKey.length)}`);
    }
 ...
 */
export async function recoverShard(encryptedShard, custodianPrivateKey, ownerVerifyKey, options) {
    const kem = options?.kem ?? HYBRID_X25519_MLKEM768;
    const info = options?.info ?? new Uint8Array(0);
    // Validate KEM ID
    if (encryptedShard.kemId !== kem.id) {
        throw new InvalidInputError(`KEM mismatch: encrypted shard uses "${encryptedShard.kemId}" but recover was called with "${kem.id}"`);
    }
    ...
    if (encryptedShard.sigAlgorithmId !== ALGORITHM_ID) {
        throw new InvalidInputError(`signature-algorithm mismatch: encrypted shard uses "${encryptedShard.sigAlgorithmId}" but only "${ALGORITHM_ID}" is supported`);
    }
    ...
    if (encryptedShard.enc.length !== kem.encSize) {
        throw new InvalidInputError(`enc must be ${String(kem.encSize)} bytes for ${kem.id}, got ${String(encryptedShard.enc.length)}`);
    }
    // Validate owner verify key
    if (ownerVerifyKey.length !== PUBLIC_KEY_BYTES) {
        throw new InvalidInputError(`ownerVerifyKey must be ${String(PUBLIC_KEY_BYTES)} bytes, got ${String(ownerVerifyKey.length)}`);
    }
    ...
    if (!verify(signature, serialized, ownerVerifyKey)) {
        throw new SignatureVerificationError('ML-DSA-65 signature verification failed: shard origin could not be authenticated');
    }
    // Step 4: Deserialize shard
    return deserializeShard(serialized);
}
//# sourceMappingURL=shard-encrypt.js.map