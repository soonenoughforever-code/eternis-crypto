 ...
 */
function tagShard(hmacKey, index, value) {
...
export async function splitKey(secret, options) {
    // Validate inputs
    if (secret.length !== SECRET_BYTES) {
        throw new InvalidInputError(`secret must be exactly ${String(SECRET_BYTES)} bytes, got ${String(secret.length)}`);
    }
    ...
    if (bytesToBigInt(secret) >= P) {
        throw new InvalidInputError('secret is out of field range (its 256-bit value must be < 2^256 - 189)');
    }
    if (!Number.isInteger(options.threshold) || options.threshold < 2) {
        throw new InvalidInputError(`threshold must be an integer >= 2, got ${String(options.threshold)}`);
    }
    if (!Number.isInteger(options.shares) || options.shares < options.threshold) {
        throw new InvalidInputError(`shares must be an integer >= threshold (${String(options.threshold)}), got ${String(options.shares)}`);
    }
    if (options.shares > MAX_SHARES) {
        throw new InvalidInputError(`shares must be <= ${String(MAX_SHARES)}, got ${String(options.shares)}`);
    }
    ...
    const shards = rawShares.map((raw) => ({
        index: raw.index,
        value: raw.value,
 ...
 */
// eslint-disable-next-line @typescript-eslint/require-await
export async function combineShards(shards) {
    // Validate inputs
    if (shards.length < 2) {
        throw new InvalidInputError(`need at least 2 shards, got ${String(shards.length)}`);
    }
    ...
    for (const shard of shards) {
        ...
        if (shard.value.length !== SECRET_BYTES) {
            throw new InvalidInputError(`shard value must be ${String(SECRET_BYTES)} bytes, got ${String(shard.value.length)} at index ${String(shard.index)}`);
        }
        if (shard.mac.length !== MAC_BYTES) {
            throw new InvalidInputError(`shard MAC must be ${String(MAC_BYTES)} bytes, got ${String(shard.mac.length)} at index ${String(shard.index)}`);
        }
    }
    ...
    if (failedIndexes.length > 0) {
        throw new ShardAuthenticationError(`HMAC verification failed for shard index(es): ${failedIndexes.join(', ')}`);
    }
    return secret;
}
//# sourceMappingURL=key-split.js.map