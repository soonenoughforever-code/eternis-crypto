 ...
 */
function encodeContext(label, parts) {
 ...
 */
export function _drawInFieldSecret(draw = randomBytes) {
 ...
 */
export async function preserve(data, custodianPublicKeys, ownerSigningKey, options) {
    ...
    if (custodianPublicKeys.length < threshold) {
        throw new InvalidInputError(`need at least ${String(threshold)} custodian public keys, got ${String(custodianPublicKeys.length)}`);
    }
    ...
    for (let i = 0; i < splitResult.shards.length; i++) {
        const encrypted = await distributeShard(splitResult.shards[i], custodianPublicKeys[i], ownerSigningKey, { kem, info: shardInfo(metadata, i) });
