...
export async function preserve(data, custodianPublicKeys, ownerSigningKey, options) {
    ...
    for (let i = 0; i < splitResult.shards.length; i++) {
        const encrypted = await distributeShard(splitResult.shards[i], custodianPublicKeys[i], ownerSigningKey, { kem });
...
export async function recover(pkg, custodianPrivateKeys, ownerVerifyKey, options) {
    ...
    for (const { index, privateKey } of custodianPrivateKeys) {
        ...
        if (!encryptedShard) {
            throw new InvalidInputError(`no encrypted shard at index ${String(index)}`);
        }
