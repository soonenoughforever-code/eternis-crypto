 ...
 */
const MAX_INVOCATIONS = 4_294_967_296;
/**
 ...
 */
const internalBindings = new WeakMap();
/**
 ...
 */
export function _internals(handle) {
...
export async function generateKey() {
    const cryptoKey = await globalThis.crypto.subtle.generateKey({ name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
 ...
 */
export async function _importRawKey(rawKey) {
    if (rawKey.length !== 32) {
        throw new Error(`AES-256 key must be exactly 32 bytes, got ${String(rawKey.length)}`);
    }
    const cryptoKey = await globalThis.crypto.subtle.importKey('raw', rawKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
    const handle = new KeyHandle();
    internalBindings.set(handle, {
        cryptoKey,
        counter: new InvocationCounter(MAX_INVOCATIONS),
