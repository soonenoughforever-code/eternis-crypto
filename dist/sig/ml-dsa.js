...
export function sign(message, secretKey) {
    if (secretKey.length !== SECRET_KEY_BYTES) {
        throw new InvalidInputError(`secretKey must be ${String(SECRET_KEY_BYTES)} bytes, got ${String(secretKey.length)}`);
    }
...
export function verify(signature, message, publicKey) {
    if (publicKey.length !== PUBLIC_KEY_BYTES) {
        throw new InvalidInputError(`publicKey must be ${String(PUBLIC_KEY_BYTES)} bytes, got ${String(publicKey.length)}`);
    }
    if (signature.length !== SIGNATURE_BYTES) {
        throw new InvalidInputError(`signature must be ${String(SIGNATURE_BYTES)} bytes, got ${String(signature.length)}`);
    }
    try {
