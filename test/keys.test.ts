...
import { generateKey, KeyHandle, _importKeyForTesting } from '../src/keys.js';
...
describe('_importKeyForTesting', () => {
  ...
  it('rejects 16-byte key', async () => {
    await expect(_importKeyForTesting(new Uint8Array(16))).rejects.toThrow();
  });

  it('rejects 33-byte key', async () => {
    await expect(_importKeyForTesting(new Uint8Array(33))).rejects.toThrow();
  });
});
